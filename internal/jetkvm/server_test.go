...
func TestRequireFFmpeg(t *testing.T) {
	// ffmpeg is present in the CI environment (installed by the workflow).
	// If it's not present locally this sub-test is skipped gracefully.
	_, err := os.LookupEnv("PATH") // always true; just ensure the env is sane
	_ = err

	t.Run("found when in PATH", func(t *testing.T) {
		orig := os.Getenv("PATH")
		// Find a directory that actually contains ffmpeg (if any).
		if requireFFmpeg() != nil {
			t.Skip("ffmpeg not available in this environment")
		}
		// Restore PATH to ensure nothing leaks.
...
func TestHandleRecordVideoValidation(t *testing.T) {
	// All of these should fail before touching the network because either
	// ffmpeg is missing or output_path is empty.

	t.Run("missing output_path", func(t *testing.T) {
		// Ensure ffmpeg appears available so we reach the output_path check.
		if requireFFmpeg() != nil {
...
func TestHandleRecordVideoDurationClamping(t *testing.T) {
	// We can't run the full handler without a real device, but we can
	// exercise the duration clamping logic directly by inspecting what
	// duration reaches makeClient (which will fail due to missing host,
	// giving us a clean exit point after the clamping code runs).
	cases := []struct {
		name  string
		input interface{}
	}{
